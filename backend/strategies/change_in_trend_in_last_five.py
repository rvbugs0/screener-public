...
def detect_swings_close_only(df, lookback=2):
    swing_highs = []
    swing_lows = []
    ...
    swing_highs.append((0,(df['time'].iloc[0], df['close'].iloc[0])))
    last_swing_high_index = 0
    highest_high_index = 0
    
    swing_lows.append((0,(df['time'].iloc[0], df['close'].iloc[0])))
    ...
    for i in range(lookback, len(df) - 0):#make this look back 0
        ...
        if all(close > c for c in prev_closes) and all(close > c for c in next_closes):
            ...
            swing_highs.append((i,(df['time'].iloc[i], close)))
            ...
            if(highest_high_index==-1):
                ...
                if(close>previous_highest_high):
                    ...
                    if(trend==0):
                        ...
                        lowest_low_index = last_swing_low_index
                        shifts_in_trend.append((i,(df['time'].iloc[i], df['close'].iloc[i])))
        ...
        # if swing low
            ...
            swing_lows.append((i,(df['time'].iloc[i], close)))
            ...
            if(trend!=0):#atleast we have some trend
                ...
                if(trend==1): #we were in an uptrend
                    ...
                    if(close<prev_low):
                        ...
                        early_shifts.append((i,(df['time'].iloc[i], df['close'].iloc[i])))
                        shift_in_trend_indices.append(i)
                        # trend = -1
                        # highest_high_index = last_swing_high_index
                        # last_swing_low_index = i
...
# original function without the ability for website to show apache plots.
...
def has_number_greater_than_x(sorted_list, x):
    # Find the insertion point for x
    index = bisect.bisect_right(sorted_list, x)
    # Check if there is any element beyond that index
    return index < len(sorted_list)


def get_trends_data(ticker_symbol,start,end,interval,changeTime=5):
    
    
    df = fetch_historical_data(ticker_symbol,start,end,interval)
    [swing_highs,swing_lows,early_shifts , shifts_in_trend,shift_in_trend_indices,trend,high_or_low,guy_responsible] = detect_swings_close_only(df, lookback=1)
    total_bars = df.shape[0]
    # we need to see if within the last 5 bars there has been a change of trend
    if has_number_greater_than_x(shift_in_trend_indices, total_bars-changeTime):
        # plot_features(df,swing_highs,swing_lows,early_shifts,shifts_in_trend,ticker_symbol)
        return [df,True,swing_highs,swing_lows,early_shifts,shift_in_trend_indices,trend,high_or_low,guy_responsible]

    # return [df,swing_highs,swing_lows,early_shifts,shifts_in_trend]
    return [df,False,swing_highs,swing_lows,early_shifts,shift_in_trend_indices,trend,high_or_low,guy_responsible]


# ticker_symbol = 'NZDJPY=X'
# interval = "1d"
# start = "2024-05-01"
# end ="2024-10-14"
# change_time = 5

# d = get_trends_data(ticker_symbol,start,end,interval,change_time)